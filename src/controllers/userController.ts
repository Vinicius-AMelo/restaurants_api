...
export function userPost(req: Request, res: Response) {
    ...
    if (name === undefined || name === '')
        ...
        res.status(200).json({ message: "Email can't be null" })
    else if (password === undefined || password === '')
        res.status(200).json({ message: "Password can't be null" })
    else if (role === undefined || role === '')
        res.status(200).json({ message: "Role can't be null" })
    else {
    ...
    if (email === undefined && password === undefined)
        ...
        UserModel.findOne({ where: { email } }).then((user) => {
            if (user !== null) {
                ...
                else res.status(200).json({ message: 'Incorrect data' })
            } else if (user === null)
                res.status(200).json({ message: 'User not found' })
        })
}
...
export function userDelete(req: Request, res: Response) {
    ...
    if (id === undefined) res.status(200).json({ message: "ID can't be null" })
        ...
        UserModel.destroy({ where: { id } }).then((removedQntd) => {
            if (removedQntd === 0)
                res.status(200).json({ message: 'ID not exists' })
