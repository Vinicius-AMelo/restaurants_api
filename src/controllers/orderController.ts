...
export function orderGetOne(req: Request, res: Response) {
    ...
    OrderModel.findOne({
        ...
        if (order === null) res.status(200).json({ message: 'Order not found' })
...
export function orderPost(req: Request, res: Response) {
    ...
    if (dishList === undefined)
        res.status(200).json({ message: "List of Dishes can't be null" })
    else if (score === undefined)
        res.status(200).json({ message: "Score can't be null" })
    else if (score < 0 || score > 5)
        res.status(200).json({ message: 'Score must be between 0 and 5' })
